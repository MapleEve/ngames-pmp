...
module OAuth2
  ...
  class Client
    ...
    # 登陆URL的地址
    def login_url(params = nil)
      connection.build_url(options[:login_url], params).to_s
    end

    # 登陆信息的地址
    def hashkey_url(params = nil)
      connection.build_url(options[:hashkey_url], params).to_s
    end
    ...
    def get_token(params, access_token_opts = {}, access_token_class = AccessToken)
      ...
      response = request(:get, token_url, opts)
      # 拿到第二层封装的实际 token 数据
      return response.body
      # realtoken = JSON::parse(response.parsed['data'].to_json)
      # access_token_class.from_hash(self, realtoken.merge(access_token_opts))
    end
    ...
    def get_info(params)
      opts = {}
      opts[:params] = params
      response = request(:get, hashkey_url, opts)
      # 返回JSON数据
      return response.body
    end
