...
class RedmineOauthControllerTest < ActionController::TestCase
  ...
  def test_oauth_ngames_with_enabled_oauth_authentification
    Setting.plugin_redmine_omniauth_ngames[:oauth_authentification] = nil
    get :oauth_ngames
    assert_response 404
  end

  def test_oauth_ngames_callback_for_existing_non_active_user
    Setting.self_registration = '2'
    user = new_user :status => User::STATUS_REGISTERED
    assert user.save
    set_response_body_stub
    get :oauth_ngames_callback
    assert_redirected_to signin_path
  end

  def test_oauth_ngames_callback_for_existing_active_user
    user = new_user
    user.activate
    assert user.save
    set_response_body_stub
  ...
  def test_oauth_ngames_callback_for_new_user_with_valid_credentials_and_sefregistration_enabled
  ...
  def test_oauth_ngames_callback_for_new_user_with_valid_credentials_and_sefregistration_disabled
    Setting.self_registration = '2'
    set_response_body_stub
    get :oauth_ngames_callback
    assert_redirected_to signin_path
  ...
  def test_oauth_ngames_callback_with_new_user_with_invalid_oauth_provider
    Setting.self_registration = '3'
  ...
  def test_oauth_ngames_callback_with_new_user_created_with_email_activation_should_have_a_token
  ...
  def test_oauth_ngames_callback_with_new_user_created_with_manual_activation
    Setting.self_registration = '2'
    set_response_body_stub
    get :oauth_ngames_callback
    assert_redirected_to :signin
  ...
  def test_oauth_ngames_callback_with_not_allowed_email_domain
    Setting.plugin_redmine_omniauth_ngames[:allowed_domains] = "twinslash.com"
    set_response_body_stub
    get :oauth_ngames_callback
    assert_redirected_to :signin
  end

  def test_oauth_ngames_callback_with_allowed_email_domain
    Setting.self_registration = '3'
    Setting.plugin_redmine_omniauth_ngames[:allowed_domains] = parse_email(@default_response_body[:email])[:domain]
    set_response_body_stub
    get :oauth_ngames_callback
    assert_redirected_to :controller => 'my', :action => 'account'
  end
