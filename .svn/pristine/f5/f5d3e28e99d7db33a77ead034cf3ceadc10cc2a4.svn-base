...
class AgileQuery < Query
  ...
  def board_statuses
    ...
    when "!"
      statuses.where("#{IssueStatus.table_name}.id NOT IN (" + status_filter_values{|val| "'#{connection.quote_string(val)}'"}.join(",") + ")").sorted
    else
      statuses.sorted
    end
  ...
  def issue_count_by_status
    @issue_count_by_status ||= self.issues.group("#{Issue.table_name}.status_id").count
  end
